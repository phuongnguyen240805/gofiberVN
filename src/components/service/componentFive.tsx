...
const ComponentFive = () => {
  ...
  const handleInitialize = () => {
    const config = {
      os: osType === 'windows' ? 'Windows Server 2019' : 'Linux (Ubuntu)',
      specs: {
        cpu: `${cpuSlider} vCPU`,
        ram: `${ramSlider} GB`,
        ssd: `${ssdSlider} GB`,
      ...
      payment: {
        method: paymentMethods[selectedPayment].name,
        discountCode: discountCode || 'None',
      },
      pricing: {
        subtotal: `${pricePerMonth.toFixed(2)} $`,
        vat: `${vat.toFixed(2)} $`,
        total: `${total.toFixed(2)} $`,
      },
    };

    console.log('--- Custom VPS Configuration ---');
    console.log(config);
    alert(
      `Đã khởi tạo cấu hình VPS: ${config.specs.cpu} CPU, ${config.specs.ram} RAM, ${config.specs.ssd} SSD`,
    );
  };
  ...
  return (
    <div className="w-full">
      <div className="mb-6 w-full rounded-2xl bg-gradient-to-r from-cyan-500 to-blue-500 p-6">
        <h1 className="text-[30px] font-bold capitalize text-white">
          Custom Cloud VPS
        </h1>
              ...
              <div className="rounded-xl shadow-sm">
                ...
                <div className="rounded-b-xl bg-white p-4">
                  ...
                  <div className="relative grid w-full grid-cols-1 gap-3 px-0 xl:grid-cols-2">
                    <div
                      className="relative h-max cursor-pointer overflow-hidden rounded-lg border border-[#e5e7eb] px-4"
                      onClick={() => setOsType('windows')}
                    >
                      <div className="flex h-full w-full items-center gap-3 py-4">
                        <div className="flex-shrink-0">
                          <img
                            src="/assets/windows_server.svg"
                            alt=""
                            width="48"
                            height="48"
                          />
                        ...
                        <div className="flex flex-1 flex-col text-start">
                          ...
                          <span className="text-small text-foreground-500 font-normal">
                            Windows Server 2019
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="mt-4">
                    {/* CPU Slider */}
                    <div
                      className="mb-5 flex w-full flex-col gap-1"
                      role="group"
                    >
                  ...
                  <div>
                    <div className="relative flex flex-col gap-2">
                      <span className="text-textSecondary relative text-sm font-semibold">
                        Invoice calculated based on:
                      </span>
                      <div className="grid grid-cols-2 flex-col flex-wrap gap-3 data-[orientation=horizontal]:flex-row xl:grid-cols-3 2xl:grid-cols-4"></div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="sticky top-2">
                <div className="box-price-config rounded-xl bg-gradient-to-t from-cyan-600 to-blue-500 px-4 py-3">
                  ...
                  <div className="my-4 rounded-xl bg-white p-3">
                    <ul>
                      <li className="border-b-1 flex flex-wrap justify-between border-[#e5e7eb] px-2 py-2 last:border-b-0">
                ...
                <Button
                  ...
                  className="mt-4 w-full !rounded-lg !bg-primary !text-white hover:!opacity-80 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Initialize Now
                </Button>
              </div>
