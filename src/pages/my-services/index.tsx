import { ProductConfig } from '@/hooks/useProduct';
...
interface ServiceMetadata {
  ...
  os?: string;
  status?: string;
  [key: string]: any; // Cho phép các trường linh hoạt khác
}
...
const MyServiceIndex = () => {
  ...
  console.log('Cart data:', cart?.cart.items);

  const productIds = useMemo(() => {
    return Array.from(new Set(cartItems.map((item) => item.product_id).filter(Boolean))) as string[];
  }, [cartItems]);

  // 2. Gọi tRPC để lấy thông tin chi tiết (bao gồm metadata gốc của product)
  const { data: productsMetadata, isLoading: isLoadingMetadata } = api.medusa.getProductRecent.useQuery(
    { ids: productIds },
  ...
  const filteredCartItems = useMemo(() => {
    ...
    return cartItems.filter((item) => {
      const originalProduct = productsMetadata?.find((p) => p.id === item.product_id);
      const serviceType = item.metadata?.service_type || originalProduct?.metadata?.service_type;

      // 1. Lọc theo nhóm sản phẩm
      const matchesTab = activeTab === 'all' || serviceType === activeTab;

      // 2. Lọc theo Trạng thái
      const matchesFilter = () => {
        if (activeFilter === 'all') return true;
        const status = item.metadata?.status || originalProduct?.metadata?.status;

        switch (activeFilter) {
          case 'active': return status === 'active';
          case 'notActivated': return status === 'unpaid' || !status;
          case 'deleted': return status === 'deleted';
          case 'aboutToExpire': {
            const expiryDateStr = item.metadata?.expiry_date || originalProduct?.metadata?.expiry_date;
      ...
      const matchesSearch = searchQuery === '' ||
        ...
        (item.metadata?.id as string)?.includes(searchQuery);

      return matchesTab && matchesFilter() && matchesSearch;
    });
  }, [cartItems, productsMetadata, activeTab, activeFilter, searchQuery]);

  const currentRecordsCount = filteredCartItems.length;
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="border-b border-[#e5e7eb] pb-4">
          ...
          <div className="mt-4 hidden sm:block">
            <a className="flex w-fit items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-medium text-white" href="/account/service/?component=1&tab=1">
        ...
        <div className="flex w-full flex-col gap-4 rounded-2xl bg-white p-4 py-6 shadow-sm !p-0 !shadow-none">
              ...
              <table className="chakra-table css-5605sr">
                <thead>
                  <tr className="overflow-hidden rounded-xl">
                    ...
                    <th className="overflow-hidden !border-b-0 bg-gray-100 !py-3 first:rounded-l-xl last:rounded-r-xl min-w-[300px]">VPS configuration</th>
                ...
                <tbody>
                  {filteredCartItems.length === 0 ? (
                    ...
                    filteredCartItems.map((item, idx) => {
                      const metadata = item.metadata as ServiceMetadata;

                      return (
                        <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                          {/* 1. Checkbox - Sticky Left 0 */}
                          <td className="sticky left-0 z-[1] min-w-[65px] bg-gray-100 py-4 px-4">
                            <input
                              type="checkbox"
                          ...
                          {/* 2. Service Id - Sticky Left 65px */}
                          <td className="sticky left-[65px] z-[1] min-w-[120px] bg-gray-100 px-4 font-medium text-gray-700">
                            #{item.id.slice(-6).toUpperCase()}
                          </td>

                          {/* 3. Service Name - Sticky Left 185px */}
                          <td className="sticky left-[185px] z-[1] min-w-[150px] bg-gray-100 px-4 font-semibold text-primary">
                            {item.title}
                          </td>

                          {/* 4. Service Label */}
                          <td className="min-w-[200px] px-4 text-gray-600">
                            {metadata?.label || item.variant?.sku || "Standard Service"}
                          </td>

                          {/* 5. IPV4 */}
                          <td className="min-w-[150px] px-4 font-mono">
                            {metadata?.ipv4 ? (
                              metadata.ipv4
                            ) : (
                              <div className="gap-2 text-gray-400 flex items-center justify-center">
                                <svg
                          ...
                          {/* 6. Invoice */}
                          <td className="min-w-[135px] px-4 font-bold text-orange-600">
                            {(item.unit_price).toFixed(2)} {cart?.currency_code?.toUpperCase() || "$"}
                          </td>

                          {/* 7. Operating System */}
                          <td className="min-w-[80px] px-4 text-sm text-gray-600">
                            {metadata?.os ? metadata : (
                              <div className='gap-2 flex items-center justify-center w-7 h-7'>
                                <img src="https://manager.gofiber.vn/images/server-icon.svg" alt="" />
                              </div>
                            )}
                          </td>

                          {/* 8. VPS Configuration */}
                          <td className="min-w-[300px] px-4 text-sm text-gray-500">
                            {item.variant?.title}
                            <ProductConfig productId={item.product?.id} />
                          </td>

                          {/* 9. Expiration Date */}
                          <td className="min-w-[200px] px-4 text-sm italic text-gray-400">
                            {item.created_at ? (
                          ...
                          <td className="min-w-[185px] px-4 text-sm text-gray-600 uppercase">
                            {cart?.payment_session?.provider_id || "-"}
                          </td>

                          {/* 12. Status */}
                          <td className="min-w-[150px] px-4 text-center">
                            <span className="rounded-md bg-yellow-100 px-2.5 py-1 text-xs font-bold text-yellow-800">
                              Pending
                            </span>
                          </td>

                          {/* 13. Auto Renew */}
                          <td className="min-w-[180px] px-4 text-center ">
                            <label className="relative inline-flex items-center cursor-pointer">
                              ...
                              // defaultValue
                              />
                              <div className="group peer bg-white rounded-full duration-300 w-8 h-4 ring-1 ring-gray-500 after:duration-300 after:bg-gray-500 peer-checked:after:bg-blue-600 peer-checked:ring-blue-600 after:rounded-full after:absolute after:h-3 after:w-3 after:top-0.5 after:left-0.5 after:flex after:justify-center after:items-center peer-checked:after:translate-x-4 peer-hover:after:scale-95" />
                            </label>
                          </td>
                          ...
                          <td className="px-4 text-center">
                            <DropdownMenu>
                              ...
                              <DropdownMenuTrigger asChild>
                                <button className="inline-flex h-8 w-8 items-center justify-center rounded-full text-gray-400 hover:bg-gray-800 transition-colors focus:outline-none">
                                  <MoreHorizontal className="h-5 w-5" />
                                </button>
                              </DropdownMenuTrigger>

                              {/* Nội dung Menu đổ xuống */}
                              <DropdownMenuContent align="end" className="w-48 border-gray-800 bg-[#111827] text-gray-300">
                                {/* Option: View Detail */}
                                <DropdownMenuItem asChild>
                                  <Link
                                    href={`/my-service/detail/${item.id}`}
                                    className="flex cursor-pointer items-center gap-2 px-3 py-2 text-xs focus:bg-blue-600 focus:text-white"
                                  >
          ...
          <div className="flex w-full flex-col-reverse items-center justify-between gap-4 lg:flex-row">
            <div className="flex w-full items-center justify-between lg:flex-1">
              <p className="text-sm text-textSecondary">There are {currentRecordsCount} records in total.</p>
